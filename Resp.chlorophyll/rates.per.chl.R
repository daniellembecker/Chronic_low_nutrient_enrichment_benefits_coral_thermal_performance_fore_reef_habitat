...
##created by Danielle Becker 03/16/2021
##edited by Danielle Becker 03/16/2021
...
# get the file path
setwd("Resp.chlorophyll/")
path.p<-"../Oct_TT.chl/" #the location of all your respirometry files
...
Sample.Info <- read.csv(file="resp_data_TT.chl.csv", header=T) #read in sample.info data
view(Sample.Info)

# load surface area data
SA <- read.csv("sample_info_TT.chl.csv", header=T) #read sample.info data
...
write.csv(Photo.R, '../Resp.chlorophyll/Output/Photo.R.chl.csv')  

#read in Photo.R file so dont need to run entire for loop again
Photo.R <- read.csv('../Resp.chlorophyll/Output/Photo.R.chl.csv')
...
Photo.R$umol.chl.ug.hr <- (Photo.R$umol.sec.corr*3600)/Photo.R$chlA.ug.sample #mmol chlugcm2 hr-1

# remove NAs and blanks
Photo.R<-Photo.R[Photo.R$BLANK==0,]

#make respiration positive
Photo.R$umol.chl.ug.hr<- Photo.R$umol.chl.ug.hr

# log the rates
Photo.R$Rate.ln<-log(Photo.R$umol.chl.ug.hr+0.1)
...
Photo.R$umol.chl.ug.hr[Photo.R$light_dark=="dark"]<-Photo.R$umol.chl.ug.hr[Photo.R$light_dark=="dark"]*-1 
lessthan <- which(Photo.R$light_dark=="dark" & Photo.R$umol.chl.ug.hr < 0)
Photo.R$umol.chl.ug.hr[lessthan] <- 0
View(Photo.R)

ggplot(Photo.R, aes(x=Temp.C, y=umol.chl.ug.hr,group = fragment.ID, col = fragment.ID))+
  geom_line(size=2)+
  geom_point()+  
  #ylim(0,1.5)+  
  facet_wrap(~ treatment*light_dark, labeller = labeller(.multi_line = FALSE))+
  ggsave(filename = "../Resp.chlorophyll/Output/lowvshigh_curves.pdf", device = "pdf", width = 10, height = 10)
 
write.csv(Photo.R, '../Resp.chlorophyll/Output/TT_Rates.csv') # export all the uptake rates
...
Photo.R2 <- Photo.R %>% 
  ...
  summarize(rates = sum(umol.chl.ug.hr), Temp.C=mean(Temp.C)) %>%
  mutate(rate.type="GP", light_dark="L") %>%
  rename(umol.chl.ug.hr = rates) %>%
...
Photo.R2$umol.chl.ug.hr[Photo.R2$umol.chl.ug.hr<0]<-0

Photo.R <- Photo.R[,c("individual.ID", "temp.Cat","treatment", "Temp.C", "umol.chl.ug.hr", "rate.type", "light_dark", "fragment.ID")]
view(Photo.R)
...
write.csv(Photo.T, '../Resp.chlorophyll/Photo.T.chl.csv') # export all the uptake rates

#check all rate types (GP, NP and R) curves
ggplot(Photo.T, aes(x=Temp.C, y=umol.chl.ug.hr, group = individual.ID, col = individual.ID))+
  geom_line(size=2)+
  geom_point()+  
  theme_bw ()+
  #ylim(0,1.5)+  
  facet_wrap(~ treatment*rate.type, labeller = labeller(.multi_line = FALSE))+ 
  ggsave(filename = "../Resp.chlorophyll/Output/initialcurves.pdf", device = "pdf", width = 10, height = 10)

#check three various rate types (GP, NP and R) individaul curves
Photo.T.GP <- Photo.T %>%
  filter(rate.type =="GP")

ggplot(Photo.T.GP, aes(x=Temp.C, y=umol.chl.ug.hr, group = individual.ID, col = individual.ID))+
  geom_line(size=2)+
  geom_point()+  
  theme_bw ()+
  #ylim(0,1.5)+  
  facet_wrap(~ treatment*rate.type*fragment.ID, labeller = labeller(.multi_line = FALSE))+ 
  ggsave(filename = "../Resp.chlorophyll/Output/GPcurves.pdf", device = "pdf", width = 10, height = 10)

Photo.T.NP <- Photo.T %>%
  filter(rate.type =="NP")

ggplot(Photo.T.NP, aes(x=Temp.C, y=umol.chl.ug.hr, group = individual.ID, col = individual.ID))+
  geom_line(size=2)+
  geom_point()+  
  theme_bw ()+
  #ylim(0,1.5)+  
  facet_wrap(~ treatment*rate.type*fragment.ID, labeller = labeller(.multi_line = FALSE))+ 
  ggsave(filename = "../Resp.chlorophyll/Output/NPcurves.pdf", device = "pdf", width = 10, height = 10)

Photo.T.R <- Photo.T %>%
  filter(rate.type =="R")


ggplot(Photo.T.R, aes(x=Temp.C, y=umol.chl.ug.hr, group = individual.ID, col = individual.ID))+
  geom_line(size=2)+
  geom_point()+  
  theme_bw ()+
  #ylim(0,1.5)+  
  facet_wrap(~ treatment*rate.type*fragment.ID, labeller = labeller(.multi_line = FALSE))+ 
  ggsave(filename = "../Resp.chlorophyll/Output/Rcurves.pdf", device = "pdf", width = 10, height = 10)
