...
mydata <- read.csv("../Resp.chlorophyll/Photo.T.chl.csv")
mydata$X <- NULL
View(mydata)
glimpse(mydata)
...
mydata$log.rate <- log(mydata$umol.chl.ug.hr + 1)  #logging and adding 0.1 because a log of zero does not exist
...
metadata <- read.csv(file="../Resp.chlorophyll/metadata.chl.csv", header=T) #read in metadata file to add site block and recovery block
...
#want to do ggplot where we look at individual curves for respiration rates of each fragment
  ...
  ggplot() +
    ...
    ylab(expression("Log " *mu*"mol chl.ug "*hr^-1*")")) +
    xlab('Temperature (ºC)') +
  ...
  ggsave(filename = "../Resp.chlorophyll/Output/respindiv.curves.chl.pdf", device = "pdf", width = 10, height = 10)
...
#want to do ggplot where we look at individual curves for photosynthesis rates of each fragment
  ...
  ggsave(filename = "../Resp.chlorophyll/Output/photoindiv.curves.chl.pdf", device = "pdf", width = 10, height = 10)
...
a <- ggplot() +
  ...
  theme_bw(base_size = 12, base_family = 'Helvetica') +
  ylab(expression(bold("Rate (log " *mu*"mol chl.ug "*hr^-1*")"))) +
  xlab('Temperature (ºC)') +
...
ggsave(filename = "../Resp.chlorophyll/Output/TPCcurves.chl.pdf", device = "pdf")
...
write.csv(Topt_data, '../Resp.chlorophyll/Output/Topt_data.chl.csv') 
...
ggsave(filename = "../Resp.chlorophyll/Output/Topt_graph.chl.pdf", device = "pdf", width = 8, height = 5)
...
c <- ggplot(data.summary.lnc, aes(x=treatment, y=lsmean, col = treatment)) +
  ...
  geom_errorbar(aes(ymax=lsmean+SE, ymin=lsmean-SE), position=position_dodge(width=0.9), width=0.1) +
  theme(legend.text=element_text(size=rel(1))) + #makes legend elements larger
  labs(x="", y=expression(bold(paste(atop("Rate at a reference temperature",  "(log " *mu*"mol chl.ug "*hr^-1*")"))))) +
  facet_wrap(. ~ rate.type, scales = "free") +
  scale_color_manual(values = wes_palette("Royal1")) +
  theme(legend.position="none", 
...
ggsave(filename = "../Resp.chlorophyll/Output/lnc_graph.chl.pdf", device = "pdf", width = 8, height = 7)
...
write.csv(Pmax_data, '../Resp.chlorophyll/Output/Pmax_data.chl.csv') # export all the uptake rates
...
#rename rate.type column
data.summary.Pmax$rate.type <- c("Gross Photosynthesis", "Gross Photosynthesis", "Respiration", "Respiration")
...
b <- ggplot(data.summary.Pmax, aes(x=treatment, y=lsmean, col = treatment)) +
  ...
  geom_errorbar(aes(ymax=lsmean+SE, ymin=lsmean-SE), position=position_dodge(width=0.9), width=0.1) +
  theme(legend.text=element_text(size=rel(1))) + #makes legend elements larger
  labs(x="", y=expression(bold(paste(atop("Maximal perfromance",  "(log " *mu*"mol chl.ug "*hr^-1*")"))))) +
  facet_wrap(. ~ rate.type, scales = "free") +
  scale_color_manual(values = wes_palette("Royal1")) +
  theme(strip.text.x = element_text(size=18, face = "bold")) +
  theme(legend.position = "none",
...
ggsave(filename = "../Resp.chlorophyll/Output/Pmax_graph.chl.pdf", device = "pdf", width = 8, height = 6)
...
ggsave(filename = "../Resp.chlorophyll/Output/holo_graphs.chl.pdf", device = "pdf", width = 10, height = 20)


#########################
#GP comparison between control and enriched

#want to do ggplot where we look at individual curves for photosynthesis rates of each fragment
mydataGP.chl <- mydata %>%
  filter(rate.type =="Gross Photosynthesis")

#change names of treatment params in data sheet
mydataGP.chl.1 <- mydataGP.chl %>% 
  mutate(treatment = recode(treatment, `Enriched` = "enriched", `Control` = "control"))

mydataGP.chl.2 <- left_join(mydataGP.chl.1, metadata)

#pmax model for dark respiration
GP.chl <- lmer(log.rate~treatment + (1|block), data = mydataGP)
anova(Pmax.GP)
summary(Pmax.GP)
